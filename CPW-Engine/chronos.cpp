...
void time_uci_go(char * command) {
    ...
    if (strstr(command, "wtime")) {
        chronos.flags |= FTIME;
        converted = sscanf(strstr(command, "wtime"), "wtime %d", &chronos.time[WHITE]);
    }
    if (strstr(command, "btime")) {
        chronos.flags |= FTIME;
        converted = sscanf(strstr(command, "btime"), "%*s %d", &chronos.time[BLACK]);
    }
    if (strstr(command, "winc")) {
        chronos.flags |= FINC;
        converted = sscanf(strstr(command, "winc"), "%*s %d", &chronos.inc[WHITE]);
    }
    if (strstr(command, "binc")) {
        chronos.flags |= FINC;
        converted = sscanf(strstr(command, "binc"), "%*s %d", &chronos.inc[BLACK]);
    }
    if (strstr(command, "movestogo")) {
        chronos.flags |= FMOVESTOGO;
        converted = sscanf(strstr(command, "movestogo"), "%*s %d", &chronos.movestogo);
    }
    if (strstr(command, "depth")) {
        chronos.flags |= FDEPTH;
        converted = sscanf(strstr(command, "depth"), "%*s %d", &chronos.depth);
    }
    if (strstr(command, "nodes")) {
        chronos.flags |= FNODES;
        converted = sscanf(strstr(command, "nodes"), "%*s %d", &chronos.nodes);
    }

    //not implemented (do an infinite search instead)
    if (strstr(command, "mate")) {
        chronos.flags |= FMATE;
        chronos.flags |= FINFINITE;
        converted = sscanf(strstr(command, "mate"), "%*s %d", &chronos.mate);
    }

    if (strstr(command, "movetime")) {
        chronos.flags |= FMOVETIME;
        converted = sscanf(strstr(command, "movetime"), "%*s %d", &chronos.movetime);
    }
...
int time_uci_ponderhit() {
    if (task != etask::TASK_PONDER) return 0;
...
void time_calc_movetime() {
    ...
    */
	...
	if ( (chronos.flags & FTIME) && !(chronos.flags & FINC) && chronos.time[sd.myside] < 1000 )
		movestogo *= 2;
...
bool time_stop_root() {
    ...
    */
    com();
    if (task == etask::TASK_NOTHING) return 1;
